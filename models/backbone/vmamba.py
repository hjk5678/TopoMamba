...
# compatible with openmmlab / semantic segmentation
class Backbone_VSSM(VSSM):
    """
    Modified Backbone_VSSM for TopoMamba.

    功能：
        1. 删除 VSSM 的 ImageNet classifier；
        2. 返回 out_indices 对应的多尺度特征；
        3. 输出统一为 [B, C, H, W]；
        4. 更稳地加载 ImageNet 预训练权重；
        5. 自动过滤 classifier/head 和 shape 不匹配参数；
        6. 打印实际加载成功的参数数量。

    注意：
        这个类仍然依赖前面的 VSSM / LayerNorm / SS2D / VSSBlock 等官方代码。
        不要单独复制这个类运行。
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        # norm_layer="ln2d" 时，VMamba 内部输出 channel_first=True
    ...
    def _rename_legacy_vmamba_keys(self, state_dict):
        """
        兼容官方 VMamba 旧 checkpoint 的参数命名和形状。

        官方 checkpoint 里可能是：
            op.x_proj_weight        [K, R+2N, D]
            op.dt_projs_weight      [K, D, R]
            op.dt_projs_bias        [K, D]

        当前简化版模型里是：
            op.x_proj.weight        [K*(R+2N), D]
            op.dt_projs.weight      [K*D, R]
            op.dt_projs_bias        [K, D]

        所以：
            x_proj_weight 需要 reshape 成二维；
            dt_projs_weight 需要 reshape 成二维；
            dt_projs_bias 不需要改名。
        """
        if not isinstance(state_dict, dict):
            return state_dict
        ...
        for k, v in state_dict.items():
            ...
            # -------------------------------------------------
            # 旧版:
            #   layers.x.blocks.y.op.x_proj_weight
            #   shape = [K, R + 2N, D]
            #
            # 新版:
            #   layers.x.blocks.y.op.x_proj.weight
            #   shape = [K * (R + 2N), D]
            # -------------------------------------------------
            if new_k.endswith(".op.x_proj_weight"):
                new_k = new_k.replace(
                    ".op.x_proj_weight",
                    ".op.x_proj.weight"
                )

                if torch.is_tensor(new_v) and new_v.dim() == 3:
                    k_group, out_per_group, in_dim = new_v.shape
                    new_v = new_v.reshape(
                        k_group * out_per_group,
                        in_dim
                    )

            # -------------------------------------------------
            # 旧版:
            #   layers.x.blocks.y.op.x_proj_bias
            #
            # 新版:
            #   layers.x.blocks.y.op.x_proj.bias
            #
            # 一般你的当前配置 bias=False，所以多数 checkpoint 没这个。
            # 如果有，也 reshape 成一维。
            # -------------------------------------------------
            elif new_k.endswith(".op.x_proj_bias"):
                new_k = new_k.replace(
                    ".op.x_proj_bias",
                    ".op.x_proj.bias"
                )

                if torch.is_tensor(new_v) and new_v.dim() > 1:
                    new_v = new_v.reshape(-1)

            # -------------------------------------------------
            # 旧版:
            #   layers.x.blocks.y.op.dt_projs_weight
            #   shape = [K, D, R]
            #
            # 新版:
            #   layers.x.blocks.y.op.dt_projs.weight
            #   shape = [K * D, R]
            # -------------------------------------------------
            elif new_k.endswith(".op.dt_projs_weight"):
                new_k = new_k.replace(
                    ".op.dt_projs_weight",
                    ".op.dt_projs.weight"
                )
                ...
                if torch.is_tensor(new_v) and new_v.dim() == 3:
                    k_group, d_inner, dt_rank = new_v.shape
                    new_v = new_v.reshape(
                        k_group * d_inner,
                        dt_rank
                    )
            ...
            # -------------------------------------------------
            # 注意：
            # 当前模型里仍然叫 op.dt_projs_bias，
            # 所以这里不要改成 op.dt_projs.bias。
            # -------------------------------------------------

            new_state_dict[new_k] = new_v

        return new_state_dict
    def _remove_classification_keys(self, state_dict):
        """
        Backbone 不需要分类头，删除这些 key，避免 unexpected 太多。
        """
        remove_prefixes = [
            "classifier.",
            "head.",
        ...
        for k in state_dict.keys():
            if any(k.startswith(p) for p in remove_prefixes):
    ...
    def _filter_mismatch_keys(self, state_dict):
        """
        strict=False 不能避免 shape mismatch 报错。
        所以这里主动过滤 shape 不一致的参数。
        """
    ...
    def load_pretrained(self, ckpt=None, key="model"):
        ...
        注意：
            这里不会加载 classifier/head。
        """
        if ckpt is None:
            print("[VMamba] pretrained is None, train backbone from scratch.")
            return
        ...
        try:
            if isinstance(ckpt, str):
                ...
                print(f"[VMamba] Load checkpoint from: {ckpt}")
            elif isinstance(ckpt, dict):
                checkpoint = ckpt
                print("[VMamba] Load checkpoint from dict.")
            else:
                raise TypeError(
            ...
            state_dict = self._strip_prefix_if_present(
                state_dict,
                prefix="module."
            )

            # 关键：先重命名旧版 VMamba checkpoint 的 key
            state_dict = self._rename_legacy_vmamba_keys(state_dict)

            state_dict = self._remove_classification_keys(state_dict)

            filtered_state, skipped_missing, skipped_shape = \
                self._filter_mismatch_keys(state_dict)
            ...
            print("=" * 80)
            print("[VMamba] Pretrained loading summary")
            print(f"  loaded tensors: {len(filtered_state)}")
            print(f"  skipped missing/unexpected keys: {len(skipped_missing)}")
            print(f"  skipped shape-mismatch keys: {len(skipped_shape)}")
            print(f"  final missing keys: {len(incompatible.missing_keys)}")
            print(f"  final unexpected keys: {len(incompatible.unexpected_keys)}")

            if len(filtered_state) > 0:
                print("[VMamba] First loaded keys:")
                for k in list(filtered_state.keys())[:30]:
                    print("   ", k)

            if len(skipped_shape) > 0:
                print("[VMamba] First shape-mismatch keys:")
                for item in skipped_shape[:20]:
                    print("   ", item)

            if len(incompatible.missing_keys) > 0:
                print("[VMamba] First final missing keys:")
                for k in incompatible.missing_keys[:20]:
                    print("   ", k)

            if len(incompatible.unexpected_keys) > 0:
                print("[VMamba] First final unexpected keys:")
            ...
            print(f"[VMamba] Failed loading checkpoint: {e}")
